...
// ---------------------------------------------------------------------------
// DataTable — filter by type: Files | Users | Content
// ⚠️  Column id "type" required with filterFn: (row, id, val) => val.includes(row.getValue(id))
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// UsersDataTable — filter by status: Active | Inactive
// ⚠️  Column id "is_active" required with filterFn: (row, id, val) => val.includes(String(row.getValue(id)))
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// DocumentRequestDataTable — filter by file_status: Pending | Approved | Rejected
// ⚠️  Column id "file_status" required with filterFn: (row, id, val) => val.includes(row.getValue(id))
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// ActivityLogDataTable — filter by log_type: Document | Content
// ⚠️  Column id "log_type" required with filterFn: (row, id, val) => val.includes(row.getValue(id))
// ---------------------------------------------------------------------------
...
export function ActivityLogDataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
  ...
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>({
    type: false,                // ← was 'log_type'
  });

  const table = useReactTable({
    data,                                               // ← raw data, no memo
    columns,
    getCoreRowModel: getCoreRowModel(),
    getPaginationRowModel: getPaginationRowModel(),
    getSortedRowModel: getSortedRowModel(),
  ...
  return (
    <div className="space-y-4">
      <DataTableToolbar
        ...
        filters={[
          {
            columnId: 'type',       // ← was 'log_type'
            title: 'Type',
            options: [
              { label: 'Documents', value: 'Document' },  // check exact casing from data
              { label: 'Users', value: 'Users' },
              { label: 'Content', value: 'Content' },
            ],
          },
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell, index) => (
                    <TableCell
                      key={cell.id}
                      className={`px-4 py-3.5 ${index === 0 ? 'pl-6' : ''} ${index === row.getVisibleCells().length - 1 ? 'pr-6' : ''}`}
                    >
                      {flexRender(cell.column.columnDef.cell, cell.getContext())}
                    </TableCell>
                  ))}
