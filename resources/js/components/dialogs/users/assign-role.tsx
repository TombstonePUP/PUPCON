...
export function AssignRole({ user, programRoles, roles, onClose }: AssignRoleDialogProps) {
    ...
    const cleanedAssignedPrograms = user.areas?.map((a) => a.levels?.programs?.program_id)?.filter((id) => validProgramIds.includes(id)) || [];

    const cleanedAssignedAreas = user.areas?.map((a) => a.area_id)?.filter((id) => validAreaIds.includes(id)) || [];
            ...
            <DialogContent className="sm:max-w-xl">
                <form onSubmit={assignUserRole} className="space-y-8">
                    <DialogHeader className="mb-4">
                        <DialogTitle className="text-foreground text-lg font-medium">Edit & Change Roles</DialogTitle>
                        <DialogDescription className="text-muted-foreground text-sm">Change roles assigned to the user</DialogDescription>
                    </DialogHeader>
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        <Checkbox
                                                                            ...
                                                                            disabled={processing}
                                                                            onCheckedChange={(checked) => onChangeArea(area, checked as boolean)}
                                                                            className="data-[state=checked]:border-[#7f1414] data-[state=checked]:bg-[#7f1414]"
                                                                        />
