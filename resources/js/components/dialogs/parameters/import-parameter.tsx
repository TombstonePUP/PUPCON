...
export default function ImportParameter({ program, area_id, onClose }: ImportParameterProps) {
    ...
    return (
        <Dialog open={true} onOpenChange={onClose}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className="text-foreground text-lg font-medium">Import Parameters</DialogTitle>
                    <DialogDescription className="text-muted-foreground text-sm">Manage parameters and benchmarks on spreadsheet</DialogDescription>
                </DialogHeader>
                <form onSubmit={importParameter} className="flex flex-col gap-10">
                    <div className="space-y-2">
                        <div className="border-border w-full rounded-lg border p-6 text-left">
                            <h3 className="text-muted-foreground mb-2 text-xs font-semibold tracking-wide uppercase">How to use</h3>
                            <ol className="text-muted-foreground list-inside list-decimal space-y-1 text-sm">
                                ...
                                <li>
                                    Open in <span className="text-foreground font-medium">Excel</span> or Google Sheets.
                                </li>
                                <li>Fill in your data (keep headers intact).</li>
                                <li>
                                    <span className="text-foreground font-medium">Save</span> and upload the file.
                                </li>
                            </ol>
                        </div>
                            ...
                            {!data.document ? (
                                <label className="border-border bg-muted hover:bg-muted/80 flex h-32 w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed">
                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p className="text-muted-foreground text-xs">CSV</p>
                                    </div>
                                    <input
                                        type="file"
                                        className="hidden"
                                ...
                                <div className="border-border bg-muted hover:bg-muted/80 flex h-32 w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed">
                    ...
                    <DialogFooter>
                        <Button className="sm:mr-auto" variant="outline" asChild>
                            <a
                                href={route('manage.area.download.template', {
                                    program_id: program.program_id,
                                    level_id: program.levels[0]?.accreditation_level_id,
