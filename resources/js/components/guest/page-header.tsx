...
export default function PageHeader({ title, breadcrumbs, quickLinks, className }: PageHeaderProps) {
    ...
    return (
        <div className={`top-0 z-30 w-full bg-white py-4 shadow-md lg:sticky ${className || ''}`}>
            <div className="mx-auto flex w-[70%] max-w-7xl items-center justify-between text-[#7f1414]">
                {/* Title */}
                {title && <h1 className="text-sm font-semibold tracking-tight">{title}</h1>}

                {/* Breadcrumbs */}
                <nav className="hidden items-center text-xs lg:flex">
                        ...
                        {quickLinks.map((link) => (
                            <button
                                ...
                                className={`relative rounded-md px-3 py-1.5 transition-all duration-300 ease-out ${
                                    activeSection === link.targetId ? 'text-[#7f1414]' : 'text-[#7f1414]/60 hover:text-[#7f1414]'
                                } `}
                            >
