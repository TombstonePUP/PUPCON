...
export function EditParameter({ parameter, program, area_id, onClose }: EditParameterProps) {
    const { data, setData, patch, processing, errors, reset } = useForm<ParameterForm>({
        area_id: area_id,
    ...
    const editParameter = (e: React.FormEvent) => {
        e.preventDefault();
        patch(
            route('manage.area.update.parameter', {
    ...
    return (
        <Dialog open={true} onOpenChange={onClose}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className="text-foreground text-lg font-medium">Edit Parameter</DialogTitle>
