...
export default function Areas({ program, area, parameterOutlineCategories, areaFormsCategories }: AreaFilesProps) {
    const [dialog, setDialog] = useState<{
        kind: 'document' | 'benchmark' | 'parameter' | null;
        action: 'view' | 'upload' | 'delete' | 'add' | 'edit' | 'import';
    ...
    const openDialog = (
        kind: 'document' | 'benchmark' | 'parameter',
        action: 'view' | 'upload' | 'delete' | 'add' | 'edit' | 'import',
        benchmark?: ParameterOutlines,
        benchmark_categories?: ParameterOutlineCategory,
        parameter?: AreaParameters
    ) => {
        setDialog({ kind, action, benchmark, benchmark_categories, parameter });
    };
                    ...
                    <div className="border-sidebar-border/70 relative space-y-5 overflow-y-auto rounded-xl border p-4">
                        ...
                        <div>
                            <ParameterAccordion
                                ...
                                resolveDocDialog={(d) => openDialog('document', d.type, d.benchmark, undefined, undefined)}
                                resolveBenchDialog={(d) => openDialog('benchmark', d.type, d.benchmark, parameterOutlineCategories, d.parameter)}
                                resolveParamDialog={(d) => openDialog('parameter', d.type, undefined, undefined, d.parameter)}
                            />
                        </div>
                    </div>
                </div>
