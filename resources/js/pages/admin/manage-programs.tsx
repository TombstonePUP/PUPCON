...
import DeleteProgram from '@/components/admin/dialogs/content/programs/delete-program';
import EndSurveyDialog from '@/components/admin/dialogs/content/programs/end-survey-dialog';
import ProgramDialog from '@/components/admin/dialogs/content/programs/program-dialog';
import ProgramLevelDialog from '@/components/admin/dialogs/content/programs/program-level-dialog';
...
export default function ManagePrograms({ programs }: ProgramsProps) {
    ...
    const programStatsConfig = [
        ...
        },
        {
            ...
            value: activePrograms.filter((p) => p.degree_type.toLowerCase().includes('bachelor')).length || 0,
        ...
        },
        {
            ...
            value: activePrograms.filter((p) => p.degree_type.toLowerCase().includes('diploma')).length || 0,
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                    <div className="bg-muted mb-4 flex h-16 w-16 items-center justify-center rounded-full">
                        {isArchived ? (
                            ...
                            <NotebookIcon className="text-muted-foreground h-8 w-8" />
                        )}
                    </div>
                    <h3 className="text-foreground mb-1 text-lg font-semibold">{isArchived ? 'No Archived Programs' : 'No Active Programs'}</h3>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {programsList.map((program) => {
                    ...
                    return (
                        ...
                        >
                            <div className="border-border bg-card hover:border-border/80 relative rounded-xl border p-6 transition-all duration-150 hover:-translate-y-0.5">
                                ...
                                <CardTitle className="text-foreground group-hover:text-primary mb-2 font-semibold capitalize transition-colors">
                                    {program.program_name}
                                </CardTitle>
                                <div className="text-muted-foreground mb-3 flex w-full truncate text-sm">
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <Dialog open={startLevelConfirmOpen} onOpenChange={setStartLevelConfirmOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-foreground mb-4 text-lg font-medium">No Levels Found</DialogTitle>
            ...
            {dialogOpen && dialogType === 'level' && dialogAction === 'add' && (
                <ProgramLevelDialog programs={programs} onClose={() => setDialogOpen(false)} selected_program_id={selectedProgramId || undefined} />
            )}
